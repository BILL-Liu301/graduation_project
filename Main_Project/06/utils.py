...
training_data_output_start_point = np.zeros((training_data_reshaped.shape[0], 1))
...
training_data_output_start_point[:, 0] = training_data_output[:, 0]
training_data_output[:, 0] = training_data_output[:, 0] - training_data_input[:, 0]

for i in range(len(training_data_input[0]) - 1):
    training_data_input[:, i + 1] = training_data_input[:, i + 1] - training_data_input[:, 0]
training_data_input[:, 0] = 0.0
...
np.savetxt('training_data_output_start_point.txt', training_data_output_start_point, fmt='%f')
...
testing_data_output_start_point = np.zeros((testing_data_reshaped.shape[0], 1))
...
testing_data_output_start_point[:, 0] = testing_data_output[:, 0]
testing_data_output[:, 0] = testing_data_output[:, 0] - testing_data_input[:, 0]

for i in range(len(testing_data_input[0]) - 1):
    testing_data_input[:, i + 1] = testing_data_input[:, i + 1] - testing_data_input[:, 0]
testing_data_input[:, 0] = 0.0

np.savetxt('testing_data_input.txt', testing_data_input, fmt='%f')
np.savetxt('testing_data_output.txt', testing_data_output, fmt='%f')
np.savetxt('testing_data_output_start_point.txt', testing_data_output_start_point, fmt='%f')

# plt.figure()
# for i in range(training_data_input.shape[0]):
#     plt.clf()
#     temp = np.append(training_data_input[i, :], training_data_output[i, :])
#     plt.plot(temp)
#     plt.pause(0.001)
